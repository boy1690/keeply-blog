...
Dropbox を開く。全部最新のファイル。昨日のバージョンなんて、ない。

![Dropbox の UI スクリーンショット。version history の入口を示す——通常はファイル右クリックメニューの 3 番目で、展開しないと見えない。タイトル：「隠されたバージョン履歴」](image-2.svg)

Google Drive を開く。バージョン履歴はメニュー 3 階層の奥。誰かに教えられないと、わからない。

![Google Drive のバージョン記録への道——ファイル → 詳細情報 → アクティビティ → バージョン記録。各ステップに矢印](image-3.svg)

会社の NAS を開く。そこにある `_v3_final.psd` こそが、あなたのバージョン履歴だ。
...
> 著者について：[創業者本名]、Keeply 創業者。
> [about](/about/) ｜ LinkedIn (Touch 4 で記入) ｜ X (Touch 4 で記入)
